...
class Model():
    def conv_bn_re(self, inputs, input_dim, out_dim, strides=(1, 1), use_relu=True, use_bn=True, k=3, is_training=True,
                   scope='conv_bn_re'):
    ...
    def residual(self, inputs, input_dim, out_dim, k=3, strides=(1, 1), is_training=True, scope='residual'):
        with tf.variable_scope(scope):
            ...
            # skip,up layer 1*1
            skip = self.conv_bn_re(inputs, input_dim, out_dim, strides=strides, use_relu=False, k=1,
    ...
    def hourglass(self, inputs, n_deep, n_res, n_dims, is_training=True, scope='hourglass_5'):
        with tf.variable_scope(scope):
            ...
            if n_deep > 1:
                low_2 = self.hourglass(low_1, n_deep - 1, n_res[1:], n_dims[1:], is_training=is_training,
    ...
    def corner_pooling(self, inputs, input_dim, out_dim, k=3, is_training=True, scope='corner_pooling'):
        with tf.variable_scope(scope):
            with tf.variable_scope('top_left'):
                ...
                top_left = self.conv_bn_re(top_left, 128, out_dim, use_relu=False, is_training=is_training,
                                           scope='top_left')

                skip_tl = self.conv_bn_re(inputs, input_dim, out_dim, use_relu=False, k=1, is_training=is_training,
                                          scope='skip')
            ...
            with tf.variable_scope('bottom_right'):
                ...
                bottom_right = self.conv_bn_re(bottom_right, 128, out_dim, use_relu=False, is_training=is_training,
                                               scope='bottom_right')

                skip_br = self.conv_bn_re(inputs, input_dim, out_dim, use_relu=False, k=1, is_training=is_training,
                                          scope='skip')
    ...
    def inter(self, input_1, input_2, out_dim, is_training=True, scope='inter'):
        with tf.variable_scope(scope):
            x_1 = self.conv_bn_re(input_1, tf.shape(input_1)[3], out_dim, use_relu=False, k=1, is_training=is_training,
                                  scope='branch_start')
            x_2 = self.conv_bn_re(input_2, tf.shape(input_2)[3], out_dim, use_relu=False, k=1, is_training=is_training,
                                  scope='branch_hourglass1')
