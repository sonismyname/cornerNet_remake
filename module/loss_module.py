...
def tag_loss(tag0, tag1, mask):
    ...
    pull = tag0 + tag1
    # push
    dist_mask = tf.reshape(mask, (tf.shape(mask)[0], 1, tf.shape(mask)[1])) + tf.reshape(mask, (
    tf.shape(mask)[0], tf.shape(mask)[1], 1))
    ...
    num2 = num * (num - 1)
    dist = tf.reshape(tag_mean, (tf.shape(tag_mean)[0], 1, tf.shape(tag_mean)[1])) - tf.reshape(tag_mean, (
    tf.shape(tag_mean)[0], tf.shape(tag_mean)[1], 1))
    # dist=-tf.pow(dist,2)
    dist = 1 - tf.abs(dist)
