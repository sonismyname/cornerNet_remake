...
class Train():
    ...
    def train_mult(self):
        ...
        lr = tf.train.exponential_decay(self.lr, steps, self.decay_step, self.decay_rate, staircase=True,
                                        name='learning_rate')
        ...
        for i in range(len(self.gpus)):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('Tower_%d' % (i)) as scope:
                    ...
                    loss, focal_loss, pull_loss, push_loss, offset_loss = self.net.loss(outs, [next_heatmaps_tl,
                                                                                               next_heatmaps_br,
                                                                                               next_tags_mask,
                                                                                               next_offsets_tl,
                                                                                               next_offsets_br])
        ...
        for step in range(self.num_steps):
            ...
            if step % 100 == 0:
                ...
                debug.test_debug(images_[0], dets_[0], debug_boxes_[0], boxes_[0], ratio_[0], self.data_train.coco,
                                 step)
    ...
    def train_single(self):
        ...
        loss, focal_loss, pull_loss, push_loss, offset_loss = self.net.loss(outs, [heatmaps_tl, heatmaps_br, tags_mask,
                                                                                   offsets_tl, offsets_br])
