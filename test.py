...
class Test():
    ...
    def test(self, sess):
        debug_dir = os.path.join(self.result_dir, "debug")
        ...
        for img_name in tqdm(img_names):
            ...
            for j in range(categories):
                ...
                if merge_bbox:
                    top_bboxes[image_id][j + 1] = soft_nms_merge(top_bboxes[image_id][j + 1], Nt=nms_threshold,
                                                                 method=2, weight_exp=weight_exp)
                else:
                    top_bboxes[image_id][j + 1] = soft_nms(top_bboxes[image_id][j + 1], Nt=nms_threshold,
    ...
    def test_debug(self, image, detections, debug_boxes, boxes, ratio, coco, step):
        ...
        for j in range(self.categories):
            ...
            if self.merge_bbox:
                ...
                top_bboxes[j + 1] = soft_nms_merge(top_bboxes[j + 1], Nt=0.5, method=2)
