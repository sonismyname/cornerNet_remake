...
from transform import random_crop, draw_gaussian, gaussian_radius, crop_image, crop_image, normalize_, color_jittering_, \
    lighting_, full_image_crop, resize_image, clip_detections
...
class Image_data():
    ...
    def get_single_data(self, queue):
        images, tags_tl, tags_br, heatmaps_tl, heatmaps_br, tags_mask, offsets_tl, offsets_br, boxes, ratio = tf.py_func(
            self.read_from_disk, [queue],
            [tf.float32, tf.int64, tf.int64, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.int64,
             tf.float32])
    ...
    def inupt_producer(self):
        ...
        self.images, self.tags_tl, self.tags_br, self.heatmaps_tl, self.heatmaps_br, self.tags_mask, self.offsets_tl, self.offsets_br, self.boxes, self.ratio = self.get_single_data(
            quene_train)

    def get_batch_data(self, batch_size):
        images, tags_tl, tags_br, heatmaps_tl, heatmaps_br, tags_mask, offsets_tl, offsets_br, boxes, ratio = tf.train.shuffle_batch(
            [self.images,
             self.tags_tl, self.tags_br, self.heatmaps_tl, self.heatmaps_br, self.tags_mask, self.offsets_tl,
             self.offsets_br, self.boxes, self.ratio],
            batch_size=batch_size, shapes=[(self.input_size[0], self.input_size[1], 3), (128), (128),
                                           (self.output_size[0], self.output_size[1], self.categories),
                                           (self.output_size[0], self.output_size[1], self.categories),
                                           (128), (128, 2), (128, 2), (128, 4), (128, 2)], capacity=100,
            min_after_dequeue=batch_size, num_threads=16)
        return images, tags_tl, tags_br, heatmaps_tl, heatmaps_br, tags_mask, offsets_tl, offsets_br, boxes, ratio
...
if __name__ == '__main__':
    ...
    data.inupt_producer()
    images, tags_tl, tags_br, heatmaps_tl, heatmaps_br, tags_mask, offsets_tl, offsets_br, boxes, ratio = data.get_batch_data(
        2)
    sess = tf.InteractiveSession()
    coord = tf.train.Coordinator()
    threads = tf.train.start_queue_runners(sess=sess, coord=coord)
    for i in range(12):
        images_, tags_tl_, tags_br_, heatmaps_tl_, heatmaps_br_, tags_mask_, offsets_tl_, offsets_br_, boxes_ = sess.run(
            [images, tags_tl, tags_br, heatmaps_tl, heatmaps_br, tags_mask, offsets_tl, offsets_br, boxes])
        for j in range(2):
